...
def enrich_5g_with_site(
    ...
    """Join site_id from df_5glist into a 5G KPI dataframe."""
    if df_kpi.empty or df_5glist.empty:
        return df_kpi
    mapping = df_5glist[["nrbts", "site_id"]].drop_duplicates()
    mapping = mapping.rename(columns={"nrbts": kpi_nrbts_col})
    merged = df_kpi.merge(mapping, on=kpi_nrbts_col, how="left")
    return merged
...
def compute_baseline_windows(
    ...
    Examples:
      30 days: PRE days 1-15,  POST days 16-30
      14 days: PRE days 1-7,   POST days 8-14
       5 days: PRE days 1-2,   POST days 3-5
       1 day : PRE = POST = same day
    """
...
def compute_daily_kpi(
    ...
    if kpi.formula_num not in df.columns:
        logger.warning("Column %s not found in dataframe", kpi.formula_num)
        return pd.DataFrame()
    ...
    agg_cols = [kpi.formula_num]
    if kpi.formula_denum and kpi.formula_denum in df.columns:
        agg_cols.append(kpi.formula_denum)

    agg = (
        df[group_keys + agg_cols]
        .groupby(group_keys)
        .sum(numeric_only=True)
        .reset_index()
    )

    if kpi.formula_denum and kpi.formula_denum in agg.columns:
        agg["kpi_value"] = _safe_ratio(
            agg[kpi.formula_num], agg[kpi.formula_denum], kpi.multiply
        )
    else:
        # Metric is already a direct value (no denominator)
        agg["kpi_value"] = agg[kpi.formula_num] * kpi.multiply

    agg["kpi_name"] = kpi.name
    agg["unit"] = kpi.unit
    return agg
...
def compute_cluster_kpi(
    ...
    num_sum = df[kpi.formula_num].sum()
    if kpi.formula_denum and kpi.formula_denum in df.columns:
        denum_sum = df[kpi.formula_denum].sum()
        if denum_sum == 0:
            return np.nan
        return (num_sum / denum_sum) * kpi.multiply
...
def compute_4g_daily_traffic(
    ...
    Column names confirmed from schema: DATA_TRAFFIC_GB, DATA_DL_TRAFFIC_GB, DATA_UL_TRAFFIC_GB.
    Falls back gracefully when DL/UL breakdown columns are absent.
    """
    if df_4g.empty:
        return pd.DataFrame()
    total_col = "DATA_TRAFFIC_GB"
    if total_col not in df_4g.columns:
        # Reconstruct total from DL+UL if available
...
def compute_5g_daily_user(
    ...
    Primary:  NR_5124A_5G_AVERAGE_NUMBER_OF_NSA_USERS_NUM / DENUM  (ratio)
    Fallback: NR_5124A_5G_AVERAGE_NUMBER_OF_NSA_USERS              (pre-computed column)
    """
    if df_day.empty:
        return pd.DataFrame()
...
def compute_4g_daily_user(df_4g: pd.DataFrame, date_col: str = "xDate") -> pd.DataFrame:
    ...
    Primary: ACTIVE_USER_NUM / ACTIVE_USER_DENUM (ratio).
    Fallback: RRC_CONNECTED_USER direct column if ratio columns absent.
    """
    if df_4g.empty:
        return pd.DataFrame()
    ...
    if traffic_col in df.columns:
        for site, grp in df.groupby(site_col):
            ...
            if pre_val > 0:
                ...
                # Traffic: higher is better → degrade when delta < -5%
...
def build_5g_failed_contributor_table(
    ...
    """
    5G contributor table — shows ALL sites that FAIL their threshold.

    Logic:
    - For each KPI that has a threshold defined, compute the average value
      per site over the full date range (no PRE/POST split).
    - A site FAILS when:
        higher_is_better=True  → avg_value < threshold
        higher_is_better=False → avg_value > threshold
    - Returns all failing sites sorted by gap (most negative = worst).
    """
    if df.empty or site_col not in df.columns:
        return pd.DataFrame()
    ...
    for kpi in kpis_with_threshold:
        ...
        for site, grp in df.groupby(site_col):
            # Compute full-range aggregate value (no window filter)
            ...
            if kpi.higher_is_better:
                ...
                gap = val - kpi.threshold  # negative = below threshold = bad
            else:
                failed = val > kpi.threshold
                gap = kpi.threshold - val  # negative = above threshold = bad
            ...
            rows.append(
                {
                    ...
                    "Actual": round(val, 4),
                    "Gap": round(gap, 4),  # negative = how far below target
                    "Unit": kpi.unit,
                    "STATUS": "🔴 Failed",
                }
            )
    ...
    # Sort: worst gap first (most negative = furthest from threshold), show all
    return result.sort_values(["Site ID", "Gap"], ascending=[True, True]).reset_index(
        drop=True
    )
